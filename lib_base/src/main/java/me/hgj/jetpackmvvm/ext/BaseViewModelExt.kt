...
fun <T> BaseViewModel.request(
    ...
    return viewModelScope.launch {
        runCatching {
            if (isShowDialog) loadingChange.showDialog.postValue(null)
            //请求体
            block()
        }.onSuccess {
            //网络请求成功 关闭弹窗
...
inline fun <reified T> Observable<BaseEntity<T>>.convert(): Observable<T> {
    return subscribeOn(Schedulers.io())
        .observeOn(AndroidSchedulers.mainThread())
        .map {
//            LogUtils.iTag(ConfigConstants.CONSTANT.TAG_ALL, "errorCode = ${it.code}")
            if (it.code != 0 && it.code != 805371949) {
                throw  ExceptionHandle.handleException(it.code, it.msg)
            }
            return@map it.data
        }
}
