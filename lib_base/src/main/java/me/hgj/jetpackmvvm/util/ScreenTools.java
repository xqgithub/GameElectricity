...
//            AutoSizeConfig.getInstance().setDesignWidthInDp(ConfigConstants.PAD_WIDTH);
            AutoSizeConfig.getInstance().setDesignHeightInDp(ConfigConstants.CONSTANT.PAD_HEIGHT);
        } else {
            AutoSizeConfig.getInstance().setBaseOnWidth(true);
            AutoSizeConfig.getInstance().setDesignWidthInDp(ConfigConstants.CONSTANT.PHONE_WIDTH);
        }
    }
...
//        float result = dpValue * scale + 0.5f;
        ...
        if (isBaseOnWidth) {
            if (isLandscape(mContext)) {
                scale = AutoSizeConfig.getInstance().getScreenWidth() * 1.0f / ConfigConstants.CONSTANT.PAD_WIDTH;
            } else {
                scale = AutoSizeConfig.getInstance().getScreenWidth() * 1.0f / ConfigConstants.CONSTANT.PHONE_WIDTH;
            }
        } else {
            if (isLandscape(mContext)) {
                scale = AutoSizeConfig.getInstance().getScreenHeight() * 1.0f / ConfigConstants.CONSTANT.PAD_HEIGHT;
            } else {
                scale = AutoSizeConfig.getInstance().getScreenHeight() * 1.0f / ConfigConstants.CONSTANT.PHONE_HEIGHT;
            }
        }
    ...
    public void getEquipmentInformation(Context mContext) {
        ...
        LogUtils.iTag(ConfigConstants.CONSTANT.TAG_ALL,
                ...
                "手机的cpu型号 =-= " + Arrays.toString(DeviceUtils.getABIs())
        );
    }
