...
object CacheUtil {
     ...
     * 是否同意协议
     */
    fun isPrivacy(): Boolean {
        val kv = MMKV.mmkvWithID("app")
        return kv.decodeBool("privacyDialog", false)
    }

    /**
     * 设置是否同意协议
     */
    fun setIsPrivacy(isLogin: Boolean) {
        val kv = MMKV.mmkvWithID("app")
        kv.encode("privacyDialog", isLogin)
    }

    /**
     * 获取保存的添加心愿购信息
     */
    fun getWished(): AddWishBus? {
        val kv = MMKV.mmkvWithID("app")
        val addwishStr = kv.decodeString("addwish")
        return if (TextUtils.isEmpty(addwishStr)) {
            null
        } else {
            Gson().fromJson(addwishStr, AddWishBus::class.java)
        }
    }
     ...
     * 设置添加心愿购信息
     */
    fun setWished(addWishBus: AddWishBus?) {
        val kv = MMKV.mmkvWithID("app")
        if (addWishBus == null) {
            kv.encode("addwish", "")
        } else {
            kv.encode("addwish", Gson().toJson(addWishBus))
        }
